...
.. code-block:: bash
    ...
    ./bin/emqctl plugins load emq_auth_clientid
...
2. 通过'./bin/emqctl'管理命令行添加用户::

   $ ./bin/emqctl users add <Username> <Password>
...
.. code-block:: bash
    ...
    ./bin/emqctl plugins load emq_auth_username
...
管理命令行'./bin/emqctl'加载卸载插件。

加载插件::

    ./bin/emqctl plugins load <PluginName>

卸载插件::

    ./bin/emqctl plugins unload <PluginName>
...
./bin/emqctl plugins load emq_auth_ldap
...
./bin/emqctl plugins load emq_auth_http
...
./bin/emqctl plugins load emq_auth_mysql
...
.. code-block:: bash
    ...
    ./bin/emqctl plugins load emq_auth_redis
...
.. code-block:: bash
    ...
    ./bin/emqctl plugins load emq_auth_mongo
...
.. code:: bash
    ...
    ./bin/emqctl plugins load emq_mod_rewrite
...
.. code:: bash
    ...
    ./bin/emqctl plugins load emq_coap
...
.. code::
    ...
    ./bin/emqctl plugins load emq_sn
...
.. code:: bash
    ...
    ./bin/emqctl plugins load emq_stomp
...
.. code-block:: bash
    ...
    ./bin/emqctl plugins load emq_sockjs
...
emq_recon插件集成recon性能调测库，'./bin/emqctl'命令行注册recon命令。
...
.. code-block:: bash
    ...
    ./bin/emqctl plugins load emq_reloader
...
.. code-block:: erlang
    ...
    emqctl:register_cmd(cmd, {emq_cli_demo, cmd}, []).

插件加载后，'./bin/emqctl'新增命令行::

    ./bin/emqctl cmd arg1 arg2
